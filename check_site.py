...
def build_email_body(added: list[str], removed: list[str], row_count: int, changed: bool, screenshot_url: str = "") -> tuple[str, str]:
    if changed:
    ...
    screenshot_html = f'<p><img src="{screenshot_url}" alt="Page screenshot" style="max-width:100%; border:1px solid #ccc;"></p>' if screenshot_url else ""
...
def main():
    ...
    if is_first_run:
        ...
        text, html_body = build_email_body(
            diff["added"], diff["removed"], len(new_rows), changed=True,
            screenshot_url=SCREENSHOT_PUBLIC_URL if screenshot_ok else ""
        )
